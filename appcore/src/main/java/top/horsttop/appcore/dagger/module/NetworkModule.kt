...
import top.horsttop.appcore.network.ParamsInterceptor
...
//                .cache(cache)
                //        .cookieJar(CookieManger(context.applicationContext))
...
//                })
                .addInterceptor(RetrofitInterceptor(context))
                .addInterceptor(ParamsInterceptor(context))
                .addInterceptor(HttpLoggingInterceptor()
...
//                .cache(cache)
    ...
    @Provides
    @Singleton
    fun ofRetrofit(okHttpClient: OkHttpClient, factory: MoshiConverterFactory): RetrofitWrapper {
        Timber.i("to ofRetrofit")
        val r1 = Retrofit.Builder()
                .baseUrl(BuildConfig.END_POINT)
                .client(okHttpClient)
                .addConverterFactory(factory)
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                .build()
        val r2 = Retrofit.Builder()
                .baseUrl(BuildConfig.END_NEW_TRAVEL)
                .client(okHttpClient)
                .addConverterFactory(factory)
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                .build()

        val r3 = Retrofit.Builder()
                .baseUrl(BuildConfig.END_NEW_TRAVEL)
                .client(okHttpClient)
                .addConverterFactory(factory)
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                .build()
        return RetrofitWrapper(r1, r2, r3)
    }
}